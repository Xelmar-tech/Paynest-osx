...
contract MyPluginSetupTest is AragonTest {
    ...
    function test_WhenDeployingANewInstance() external view {
        ...
        assertTrue(
            impl != address(0),
            "Implementation address should not be zero"
        );
    ...
    function test_WhenPreparingAnInstallation() external {
        ...
        ) = pluginSetup.prepareInstallation(address(dao), installParameters);
        ...
        assertEq(
            ...
            initialNumber,
            "Plugin should be initialized with correct number"
        );
        assertEq(
            address(pluginInstance.dao()),
            address(dao),
            "Plugin should be initialized with correct DAO"
        );

        // Verify setup data
        assertEq(
            preparedSetupData.helpers.length,
            0,
            "Should not have any helpers"
        );
        assertEq(
    ...
    function test_WhenPreparingAnUninstallation() external {
        ...
        assertEq(
            permissions.length,
            0,
            "Should not have any permissions to remove"
        );
    }
